...
def get_row_range_wo(Cell, row, col, col_step = 0):
    ...
    list_sets = [x.range for i,x in enumerate(Cell.Parent.Cells[row][:]) if (i!=col and i!=col+col_step)]
...
def get_col_range_wo(Cell, row, col, row_step = 0):
    ...
    list_sets = [x.get_range() for i,x in enumerate(Cell.Parent.Cols[col].Cells) if (i!=row and i!=row+row_step)]
    full_set = set().union(*list_sets)
    return full_set
